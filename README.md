...
*   **Зависимости**:
    *   `implementation`: Зависимость доступна только внутри текущего модуля. Это предпочтительный способ, так как он ускоряет сборку.
    *   `api`: Зависимость становится частью API модуля и транзитивно доступна модулям, которые зависят от текущего.
    *   `kapt`: Для подключения процессоров аннотаций (старый способ).
